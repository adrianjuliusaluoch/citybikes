...
def fetch_citybikes_network(network_id: str) -> pd.DataFrame:
    ...
    stations_table["collected_at"] = pd.Timestamp.now("UTC")
    return stations_table


# --- Google Sheets ---
def connect_sheets(max_attempts=3, wait_seconds=10):
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt == max_attempts:
                raise
            print(f"Sheets API error on attempt {attempt}/{max_attempts}: {error}. "
